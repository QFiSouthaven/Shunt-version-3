...
import { XMarkIcon, MailboxIcon, CopyIcon, CheckIcon, DocumentArrowDownIcon } from '../icons';
        ...
        <div className="flex flex-col h-full">
            <header className="p-4 border-b border-gray-700 flex-shrink-0">
                <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        <button onClick={() => downloadFile(file)} className="flex items-center gap-2 text-sm px-3 py-1.5 rounded-md transition-all duration-200 bg-gray-700/50 text-gray-300 hover:bg-gray-700/80">
...
const MailboxModal: React.FC<MailboxModalProps> = ({ isOpen, onClose }) => {
    const { files, markAsRead, clearMailbox, downloadAllFiles } = useMailbox();
                        ...
                        <header className="flex items-center justify-between p-4 border-b border-gray-700 flex-shrink-0">
                            <div className="flex items-center gap-3">
                             ...
                             <div className="flex items-center gap-4">
                                {files.length > 0 && (
                                    <>
                                        <button onClick={downloadAllFiles} className="flex items-center gap-1 text-sm text-cyan-400 hover:text-cyan-300 transition-colors">
                                            <DocumentArrowDownIcon className="w-4 h-4" /> Download All
                                        </button>
