...
import { SparklesIcon, ErrorIcon, TerminalIcon, CpuChipIcon, GlobeAltIcon, BoltIcon } from '../icons';
...
const ChatMessage: React.FC<ChatMessageProps> = ({ role, content, isLoading, provider, onExecuteCode }) => {
  ...
  return (
    <div className={wrapperClasses}>
      ...
      <div className="flex flex-col">
        <div className={`flex items-center gap-2 font-semibold text-sm text-gray-400 mb-1 px-2 ${isUser ? 'flex-row-reverse' : ''}`}>
            {author}
            {provider && (
                <span className="text-[8px] uppercase tracking-tighter bg-black/40 px-1.5 py-0.5 rounded border border-gray-800 flex items-center gap-1">
                    {provider === 'local' && <CpuChipIcon className="w-2 h-2 text-emerald-500" />}
                    {provider === 'cloudflare' && <GlobeAltIcon className="w-2 h-2 text-orange-500" />}
                    {provider === 'gemini' && <BoltIcon className="w-2 h-2 text-cyan-500" />}
                    {provider}
                </span>
            )}
        </div>
